...
class MotionAnalyzer:
    """Optical flow + FFT motion analysis for seizure detection."""
    ...
    def get_motion_score(self, frame):
        """Calculate motion score from optical flow + FFT."""
    ...
    def _analyze_spectrum(self):
        ...
        filtered_data = self._bandpass_filter(data, fps)
        ...
        if np.any(mask_seizure):
            peak_idx = np.argmax(power[mask_seizure])
            self.last_frequency = freqs[mask_seizure][peak_idx]
    ...
    def get_frequency(self):
        """Return last detected dominant frequency."""
